    ...
    def vend(self):
        if self.stock==0:
            ...
            if self.balance>self.price:
                ...
                a=self.balance
                self.balance=0
                return(f'Here is your '+self.stuff+f' and ${a-self.price} change.')
            else:
                if self.balance==self.price:
                    self.stock-=1
                    self.balance-=self.price
                    return(f'Here is your '+self.stuff+'.')
                else:
                    return(f'Please add ${self.price-self.balance} more funds.')
    def restock(self,num):
        self.stock+=num
        return(f'Current {str(self.stuff)} stock: {self.stock}')
...
def store_digits(n):
    ...
    def getremain(n):
        if length(n)<1:
            ...
            return n%(10**(length(n)-1))
    if n<10:
        return Link(n)
    return Link(uppermost(n),store_digits(getremain(n)))
...
def two_list(vals, counts):
    ...
    if counts[0]==1:
        ...
        copy=list(counts)
        copy[0]-=1
        return Link(vals[0],two_list(vals,copy))
