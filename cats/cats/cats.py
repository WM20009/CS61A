...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def f(s):
        a=split(lower(remove_punctuation(s)))
        for i in subject:
            flag=any([i==x for x in a])
            if flag==True:
                return True
        return False
    return f
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    a={x:diff_function(typed_word,x,limit) for x in word_list}
    b=[x for x in a if a[x]==min(list(a.values())) and a[x]<=limit]
    if not b:
        return typed_word
    else:
        return b[0]
...
def report_progress(typed, source, user_id, upload):
    ...
    pro=len(typed)/len(source)
    for i in range(len(typed)):
        if typed[i]!=source[i]:
            pro=i/len(source)
            break
    upload({'id': user_id, 'progress': pro})
    return pro
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[x[i]-x[i-1] for i in range(1,len(x))]for x in tpp]  # You may remove this line
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    def min_time(times,i):
        tmp=[x[i] for x in times]
        return tmp.index(min(tmp))
    a=[[]for i in range(len(times))]
    for i in range(len(words)):
        a[min_time(times,i)].append(words[i])
    return a
