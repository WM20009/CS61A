...
def interleaved_sum(n, odd_func, even_func):
    ...
    def oddcal(k):
        if k>n:
            ...
            return odd_func(k)+evencal(k+1)
    def evencal(k):
        if k>n:
            return 0
        else:
            return even_func(k)+oddcal(k+1)
    return oddcal(1)
...
def count_dollars(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def decide(remain,maxbill):
        if maxbill==1 or remain==0:
            return 1
        if remain<0 :
            return 0
        return decide(remain,next_smaller_dollar(maxbill))+decide(remain-maxbill,maxbill)
    return decide(total,100)
